...
app.factory('friendsFactory', ['$http', function($http) {
  ...
  function FriendsFactory(){
    ...
    this.update = function(editedFriend, callback){ // what parameters do we need?
      $http
        .put('/friends/' + editedFriend._id, { firstName: editedFriend.firstName, lastName: editedFriend.lastName })
    ...
    this.delete = function(id, callback){// what parameters do we need?
      $http.delete('friends/' + id).then(function(return_data){
        callback(return_data.data.friend);
      })
